...
if (!$_SESSION['payment-report']) {
    redirect('/payment-report');
}
...
$pdf->cell(0, 10, 'Sol & Luna Apparel Payments Report', 0, 1, 'L');

# Optionally, add a tagline or description
$pdf->SetXY($start_date ? 70 : 80, 20);
$pdf->SetFont('Arial', '', 10);
$pdf->cell(0, 10, 'Payments report for ' . $date_message , 0, 1, 'L');
...
// Table Header
$header = [
    'Created At', 'Order Id', 'Email', 'Status', 'Payment', 'Payment Status', 'Total Amount'
];
...
// Table Data
$pdf->SetFont('Arial', '', 7);
foreach ($_SESSION['payment-report'] as $row) {
    foreach ($header as $i => $col) {
